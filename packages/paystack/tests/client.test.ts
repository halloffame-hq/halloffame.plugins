...
vi.mock('@paystack/inline-js', () => ({
  default: class {
    resumeTransaction(accessCode: string, callbacks: NonNullable<typeof inline.callbacks>) {
...
describe('Paystack browser client', () => {
  it('verifies InlineJS success in realtime', async () => {
    ...
    expect(onVerified).toHaveBeenCalledWith({ status: 'paid' }, 'hof_reference')
  })
  ...
  it('creates and recognizes a nonce-bound callback URL', () => {
    ...
    expect(paymentCallbackFromUrl(redirected.toString(), storage)).toMatchObject({
      reference: 'hof_reference',
    })
  ...
  it('verifies a detected callback and removes payment query parameters', async () => {
    ...
    expect(verify).toHaveBeenCalledWith('hof_reference')
    expect(replaceState).toHaveBeenCalledWith(null, '', 'https://app.example.com/account/billing')
    stop()
