...
export function paymentCallbackFromUrl(
  ...
  if (
    storage &&
    storage.getItem(`${CALLBACK_STORAGE_PREFIX}${token}`) !== "pending"
  )
    return null;
...
export function listenForPaystackCallback(
  options: PaymentCallbackListenerOptions,
): () => void {
  ...
  const check = async () => {
    ...
    try {
      const verification: PaymentVerification = await options.verify(
        callback.reference,
      );
