...
export async function promptForTheme(
  ...
  document.shape.radii = Object.fromEntries(
    RADIUS_KEYS.map((key) => [key, document.shape.radii?.[key] ?? document.shape.radius]),
  ) as unknown as ThemeRadii

  document.identity.name = await valid(prompt, 'Application name', document.identity.name, 80)
  ...
  document.identity.tagline = await valid(prompt, 'Tagline', document.identity.tagline, 160)
  ...
  document.auth.title = await valid(prompt, 'Authentication title', document.auth.title, 160)
  ...
  const mode = await prompt.choice('How much of the theme should be customized?', [
    { name: 'Essential brand colors', value: 'essential' },
    { name: 'Every theme token', value: 'complete' },
  ])

  const paletteKeys = Object.keys(document.colors.light) as Array<keyof ThemePalette>
  ...
  if (mode === 'complete') {
    document.typography.sans = await valid(prompt, 'Body font stack', document.typography.sans, 200)
    ...
    for (const key of Object.keys(document.shape.radii) as Array<keyof ThemeRadii>) {
...
export async function promptForSystemOptions(
  ...
  const values = new Set(
    (await prompt.checkbox('System Options', [
