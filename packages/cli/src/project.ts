...
export async function detectHallOfFameProject(cwd = process.cwd()): Promise<HallOfFameProject> {
  ...
  const kind = (Object.keys(PROJECTS) as ProjectKind[]).find(
    (candidate) => manifest.name && PROJECTS[candidate].names.includes(manifest.name),
  )
  ...
  if (kind && manifest.name) {
    ...
    if (markerChecks.every((check) => check.exists)) return { kind, name: manifest.name, root }

    const missing = markerChecks.filter((check) => !check.exists).map((check) => check.marker)

    throw new Error(`Hall of Fame ${kind} project is missing required files: ${missing.join(', ')}`)
  }
