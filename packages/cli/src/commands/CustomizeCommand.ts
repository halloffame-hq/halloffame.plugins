...
export class CustomizeCommand extends Command {
  ...
  async handle(): Promise<void> {
    ...
    this.info(
      `Detected Hall of Fame ${project.kind} project at ${project.root}.`,
    );
    ...
    try {
      ...
      const themeName = await this.ask(
        "Theme version name",
        `${active.name} customization`,
      );

      if (!themeName.trim() || themeName.trim().length > 120) {
        throw new Error(
          "Theme version name must be between 1 and 120 characters.",
        );
      }

      const document = await promptForTheme(this, active.document);
      const settings = await promptForSystemOptions(
        this,
        await repository.systemOptions(),
      );

      const activate = await this.confirm(
        "Apply this customization immediately?",
        true,
      );

      const theme = await repository.save(
        themeName.trim(),
        document,
        settings,
        activate,
